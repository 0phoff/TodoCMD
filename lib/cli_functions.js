    ...
    lists.forEach(index => {
      ...
      if (!list.length)
        var p = Promise.resolve({ delete: true });
         ...
         p = inquirer.prompt({type: 'confirm', name: 'delete', message: `${list.title} is not empty! Remove anyway?`, default: false});
  ...
  if (search)
  {
    ...
    if (listindices.length == 1)
      ...
      listindices.forEach(index => {
        let list = project.getList(index);
        question.choices.push({value: index, name: list.title});
      });
  }
  else
  {
    project._content.forEach((list, index) => question.choices.push({value: index, name: list.title}));
  }
...
function getItems(project, listsearch, type, message, search, disable, all)
{
  ...
  if (search)
  {
    lists.forEach(list => {
      ...
      if (itemindices.length)
        question.choices.push(new inquirer.Separator(list.title));
      itemindices.forEach(index => {
        let item = list.getItem(index);
        question.choices.push({ value: {list, index}, name: item.value });
      });
    });
