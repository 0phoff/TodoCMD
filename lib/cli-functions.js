...
function newProject(options) {
  const {filename, exists} = getFile(options, true);
...
function list(options) {
  ...
  if (!exists) {
    const retry = getFile({file: '.' + filename});
    exists = retry.exists;
    if (!exists) {
...
function listRm(options) {
  ...
  return md.readFile(filename)
    .then(project => {
      ...
      return getLists(project, 'specialCheckbox', 'Pick the list you want to remove:', search, !options.limit)
        .then(listindices => {
...
function itemAdd(options) {
  ...
  return md.readFile(filename)
    .then(project => {
      ...
      return getLists(project, 'list', 'Pick a list:', list, desc)
        .then(listindex => {
          let p;
          if (options.argv.length === 0) {
...
function itemRm(options) {
  ...
  return md.readFile(filename)
    .then(project => {
      ...
      return getItems(project, list, desc, 'specialCheckbox', 'Pick the items you want to remove:', search)
        .then(itemindices => {
...
function itemMark(value, options) {
  ...
  return md.readFile(filename)
    .then(project => {
      ...
      return getItems(project, list, desc, 'specialCheckbox', msg, search, disable, options.all)
        .then(itemindices => {
          if (itemindices.length === 0) {
            return Promise.reject(new Error('No items selected'));
          }
...
function getFile(options, parseArgv) {
  ...
  else if (parseArgv && options.argv.length !== 0) {
...
function getLists(project, type, message, search, desc) {
  if (project.length === 0) {
...
function getItems(project, listsearch, listDesc, type, message, search, disable, all) {
  if (project.length === 0) {
    return Promise.reject(new Error('This project does not contain any lists'));
  }
